...
def test_attach():
    ...
    class OneMetricDummyModel(DummyModel):
        def configure_metrics(self) -> Union[AttachedMetric, List[AttachedMetric]]:
            return AttachedMetric("DummyMetric", RetrievalPrecision(), TrainStage.TRAIN)
    ...
    class TwoMetricsDummyModel(DummyModel):
        def configure_metrics(self) -> Union[AttachedMetric, List[AttachedMetric]]:
            return [
                AttachedMetric("DummyMetric_1", RetrievalPrecision(), TrainStage.TRAIN),
                AttachedMetric(
                    "DummyMetric_2", RetrievalReciprocalRank(), TrainStage.VALIDATION
                ),
            ]
...
def test_attached_metric():
    metric = AttachedMetric("DummyMetric_1", RetrievalPrecision(), TrainStage.TRAIN)
    assert len(metric.stages) == 1

    metric = AttachedMetric(
        "DummyMetric_2", RetrievalPrecision(), [TrainStage.TRAIN, TrainStage.VALIDATION]
    )
    ...
    metric = AttachedMetric(
        "DummyMetric_4", RetrievalReciprocalRank(), TrainStage.TRAIN
    )
    ...
    with pytest.raises(ValueError):
        AttachedMetric(
            "DummyMetric_5",
            RetrievalReciprocalRank(compute_on_step=False),
            TrainStage.TRAIN,
        )
